...
class AgentBase:
    ...
    def init(self, net_dim, state_dim, action_dim, learning_rate=1e-4, _if_per_or_gae=False, gpu_id=0):
        ...
        self.cri_target = deepcopy(
            self.cri) if self.if_use_cri_target else self.cri
        self.act_target = deepcopy(
            self.act) if self.if_use_act_target else self.act

        self.cri_optim = torch.optim.Adam(self.cri.parameters(), learning_rate)
        self.act_optim = torch.optim.Adam(
            self.act.parameters(), learning_rate) if self.ClassAct else self.cri
    ...
    def update_net(self, buffer, batch_size, repeat_times, soft_update_tau) -> (float, float):
        ...
        for _ in range(int(buffer.now_len / batch_size * repeat_times)):
            obj_critic, state = self.get_obj_critic(
                buffer, batch_size)  # critic loss
            self.optim_update(self.cri_optim, obj_critic)
            self.soft_update(self.cri_target, self.cri, soft_update_tau)

            action_pg = self.act(state)  # policy gradient
            # actor loss, makes it bigger
...
class AgentTD3(AgentBase):
    ...
    def get_obj_critic(self, buffer, batch_size) -> (torch.Tensor, torch.Tensor):
        with torch.no_grad():
            ...
            next_q = torch.min(
                *self.cri_target.get_q1_q2(next_s, next_a))  # twin critics
            q_label = reward + mask * next_q

        q1, q2 = self.cri.get_q1_q2(state, action)
        obj_critic = self.criterion(q1, q_label) + \
            self.criterion(q2, q_label)  # twin critics
        return obj_critic, state
...
class AgentSAC(AgentBase):
    ...
    def init(self, net_dim, state_dim, action_dim, learning_rate=1e-4, _if_use_per=False, gpu_id=0, env_num=1):
        ...
        self.alpha_optim = torch.optim.Adam(
            (self.alpha_log,), lr=learning_rate)
    ...
    def update_net(self, buffer, batch_size, repeat_times, soft_update_tau):
        ...
        for _ in range(int(buffer.now_len * repeat_times / batch_size)):
            ...
            obj_critic = self.criterion(
                q1, q_label) + self.criterion(q2, q_label)
            self.optim_update(self.cri_optim, obj_critic)
            self.soft_update(self.cri_target, self.cri, soft_update_tau)

            '''objective of alpha (temperature parameter automatic adjustment)'''
            action_pg, log_prob = self.act.get_action_logprob(
            ...
            obj_actor = -(torch.min(*self.cri_target.get_q1_q2(state,
                          action_pg)) + log_prob * alpha).mean()
            self.optim_update(self.act_optim, obj_actor)
...
class AgentPPO(AgentBase):
    ...
    def update_net(self, buffer, batch_size, repeat_times, soft_update_tau):
        with torch.no_grad():
            ...
            buf_r_sum, buf_advantage = self.get_reward_sum(
    ...
    def get_reward_sum_gae(self, buf_len, ten_reward, ten_mask, ten_value):
        ...
        for i in range(buf_len - 1, -1, -1):
            ...
            pre_r_sum = buf_r_sum[i]
            buf_advantage[i] = ten_reward[i] + ten_mask[i] * \
                (pre_advantage - ten_value[i])  # fix a bug here
            pre_advantage = ten_value[i] + \
